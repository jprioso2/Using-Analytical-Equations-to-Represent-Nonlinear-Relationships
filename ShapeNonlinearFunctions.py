...
1.	Create a text file (with .txt file extension) for each table function that 
    ...
    Input2, Input3 and so on. Each text file must include the following column 
    header names in row 1 (include only the text after the colon for each 
    column and where the columns are separated by one tab - Tab delimited text 
    file).
    •	Column1: x
    •	Column2: y
...
2.	Save the text file in the same location (i.e., folder) as the Python script. 
    Include the file path in line 133 of the code and add r before the path. 
    For instance, "dir_path = r"C:\Users\". 
...
4.	Run the script to generate the results. By default the code estimates and 
    ...
    in column5 of each text input file. In addition, to display the results of 
    only the best fitting analytical functions per each table funcion, you can 
    ...
    If you activate lines 509-513 of the code, the code will display the next 
    prompt by each table function asks you to: “Graph and results to display 
    ...
    constraints will be displayed. In general, we recommend displaying the
    results for all of the analytical equations that meet the constraints. 
    Enter a number within the range provided in the prompt and then press 
    Return.
...
5.  5.	The estimation results for the number of analytical equations you 
    ...
    along with the original table function. The code creates a subfolder called 
    ...
    equation we chose to replace 15 of the 22 table functions. For the 
...
Copy the path of the folder were you have the text files. Include it between 
" " and using a r before. i.e., r"C:\..."
"""
dir_path = r"C:\"
