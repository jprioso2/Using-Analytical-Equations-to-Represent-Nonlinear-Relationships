...
3.  In the text file add the x-values and y-values in the corresponding columns 
    ...
    Finally, add the number of graphs you want to display. The maximun number of 
    graphs to display is 6 if all analytical equations are feasible. You can 
    leave this value empty or add 0 to display all feasible analytical equations. 
    Ensure you do not have any extra empty rows after the last number of the 
    table function in the text file, i.e., check that the previous rows only 
    contain the final number of your table function. Save each change made 
    in the text file. 

Run the Python Script code and then respond to prompts for information when 
running the code:
4.  Run the code to generate the results. By default the code estimates and 
    reports results for all of the feasible analytical equations. 
    However, if you want to display the results of only the best 
    fitting analytical funtions, you can indicate the number of analytical fuctions 
    to display in column5. In addition, to display the results of only the best
    fitting analytical functions, you can activate lines 458-464 of the code 
    by eliminating the # at the beginning of each of these lines (but be sure 
...
for xx in range(1,count+1): #Iterate throught the Txt files saved in the folder
   ...
   if Input > len(RMSESsort):
    ...
    #Indicate how many graphs to display considering the available options.
   # if RMSESsort.size>0:
